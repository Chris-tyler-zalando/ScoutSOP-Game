ZALANDO SCOUT — SYSTEM UPDATE V2.60c
====================================
...
V2.60c additions and corrections:
...
- Kept the advanced map-builder controls from V2.60c.
...
V2.60c:
- Boss victory score board now stretches score.png to fit the whole text panel.
- Scoreboard is taller and text positions are moved inside the board.
- Stops success/boss one-shot audio when leaving the boss screen so gameplay music does not overlap.
- Boss death/gameover now works in bossFight mode; running out of hearts goes to Game Over.
- Ramming Ivan now only damages him when actively driving upward into him, preventing passive/automatic heart drain.
- Boss damage is locked:
  * shoe/offline-stock = exactly 1 Ivan heart
  * ram = exactly 2 Ivan hearts
  * Ivan fireball = exactly 1 player heart
- Shoes are 30% smaller.
- Shoe hit detection still uses a swept vertical check so fast throws do not pass through Ivan.
- Fireball glow changed to a radial gradient so it fades cleanly instead of showing a blocky red glow.
- Fireball frame animation now advances from an explicit frame counter.
- Boss fight starts from the player's current max-heart capacity.

V2.60c:
- Fixed loading-assets freeze caused by the previous ZIP not including the full base asset set.
- Restored the required base images from images.zip into the assets folder, including score.png, background1.jpg, title.png, inventory images, box4.png, etc.
- Added score to the optional asset guard as an extra safety fallback.
- Asset loading failure message now explicitly tells you to check the assets folder.
- Kept the V2.60c boss logic/music fixes.
Bundled newer direct assets found: pants.webp, shoes.webp, tops.webp, conveyor2.png, conveyor.png, boss1.webp, bossbg.jpg, bossbg1.jpg, car.png, car.webp, car_battle_sheet.png, fireball.webp, elevator.png
Newer optional assets not found locally while packaging: box.png, scanner.png, scanner2.png, scanner3.png, welcome3.jpg, conveyor.jpg, minimap.webp, printers.png, bathroom.png, table.png, table2.png, table3.png, smallbox.png, smallbox2.png, smallbox3.png, palletjack.png, officeframe.webp, pcmenu.jpg, jira.jpg, error.jpg, scoticon.png, fire.webp, fire.png, qs.png, qs2.png, qs2.jpg, clothesdamaged.png, slbox.png, baseoffice.jpg, shoe.png, shoe1.png, shoe2.png, shoe3.png, it2.jpg

V2.60c:
- Replaced the image loader so the game can no longer stay stuck forever on Loading warehouse assets.
- Every image load has a timeout and then tries the next fallback filename.
- If all filenames fail, the game uses a small generated placeholder image instead of blocking startup.
- Loading screen now shows progress like Loading warehouse assets... 10/63.
- Image URLs now include ?v=2.60b cache-busting so GitHub Pages/browser cached 404s should not keep breaking the page after upload.
- Kept V2.60c bundled assets and V2.60c boss logic/music fixes.

V2.60c:
- Fixed startup crash: closeCockpitHelp was referenced by event listeners but missing.
- Added a robust closeCockpitHelp() implementation.
- Added visible startup error reporting so future JavaScript startup crashes show on the loading screen instead of appearing like an asset-loading freeze.
- Kept V2.60b never-stuck asset loader and V2.60 boss logic/music fixes.
